(function ($) {
	...
	UmamiCharts.register({
		...
		render: function (data) {
			var items = data.browsers;
			if (!items || !Array.isArray(items) || items.length === 0) return '';

			var html = '<div class="umami-section" id="umami-section-browsers">';
			html += '<div class="umami-section-header">';
			html += '<h3><i class="bi bi-browser-chrome"></i> Browser</h3>';
			html += '</div>';
		...
		_renderList: function (items) {
			...
			items.slice(0, 10).forEach(function (item) {
				...
				html += '<i class="bi ' + H.getBrowserIcon(item.x) + ' umami-list-icon"></i>';
